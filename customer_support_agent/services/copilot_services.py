...
class SupportCopilot:
    def __init__(self, settings: Settings):
        if not settings.openai_api_key:
            raise RuntimeError(
                "OPENAI_API_KEY is missing. Add it in .env before generating drafts."
            )
    ...
    def generate_draft(self, ticket: dict[str, Any], customer: dict[str, Any]) -> dict[str, Any]:
        ...
        kb_hits = self.rag.search(query=query, top_k=self._settings.rag_top_k) or []
        ...
        agent_result = self._agent.invoke(
            ...
            config={
                ...
                # A support draft needs at most a few tool calls. Bounding the
                # loop prevents a malformed tool cycle from holding the HTTP
        ...
        if isinstance(content, list):
            ...
            for item in content:
                if isinstance(item, str):
                    ...
                    # Some providers return structured text blocks, sometimes
                    # with a signature field used for tool calling. Only
    ...
    def _build_system_prompt(self, memory_hits: list[dict[str, Any]], kb_hits: list[dict[str, Any]]) -> str:
        return (
            ...
            "Output rules:\n"
            "1) Return only a complete, customer-ready plain-text email; do not include commentary or labels outside the email.\n"
            "2) Use this exact structure: a `Subject: Re: ...` line, a blank line, `Hi <customer name>,`, a blank line, the email body, a blank line, a professional sign-off, and `Support Team`.\n"
            "3) Address the customer by the name supplied in the customer details. Never use `Unknown`; use `there` only when no name is available.\n"
            "4) Start the body with empathy and direct acknowledgement, then provide clear next steps or a resolution path.\n"
            "5) Reference KB/tool facts when relevant, without exposing internal chain-of-thought.\n"
            "6) Keep the complete email under 180 words unless more detail is necessary."
        )
    ...
    def _build_user_prompt(ticket: dict[str, Any], customer: dict[str, Any]) -> str:
        return (
            ...
            "Create the customer-ready email now. Personalize the greeting with the customer's name "
            "and use the ticket subject in the Subject line. Use tools when the ticket likely needs "
            "billing, plan, or account-level checks."
        )
    ...
    def _fallback_generate_text(
        ...
        fallback_system = (
            "You are an AI support copilot. Produce only the final customer-facing plain-text email. "
            "No tool calls. The email must contain: `Subject: Re: <ticket subject>`, a blank line, "
            "`Hi <customer name>,`, a blank line, a concise empathetic body with next steps, a blank line, "
            "and the sign-off `Best,` followed by `Support Team`. Use the supplied customer name; use "
            "`there` only if no name is available."
        )
        fallback_user = (
            ...
            "Write the complete email in the required structure, personalized with the customer's name."
        )
    ...
    def _deterministic_fallback(
        ...
        return (
            f"Subject: Re: {ticket.get('subject', 'your support request')}\n\n"
            f"Hi {customer_name},\n\n"
